...
```bash
source .env && cast block-number --rpc-url "$AGENT_AUDIT_RPC_URL"
```

- deployed bytecode:

```bash
source .env && cast code <address> --rpc-url "$AGENT_AUDIT_RPC_URL"
```

- storage slot:

```bash
source .env && cast storage <address> <slot> --rpc-url "$AGENT_AUDIT_RPC_URL"
```

- read-only contract call:

```bash
source .env && cast call <contract> "balanceOf(address)(uint256)" <arg> --rpc-url "$AGENT_AUDIT_RPC_URL"
```
