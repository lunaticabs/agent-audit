...
- The OpenRouter runtime config disables Codex's built-in web search for this model, and the SDK runner also passes `webSearchMode: "disabled"` for each thread. Enabling it causes Codex to request an OpenRouter server tool, which is unavailable for the configured routing policy; local shell and patch tools remain enabled.
