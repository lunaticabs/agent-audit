...
1. Ensure `slither_project/build_manifest.json` exists. Normally `$workspace` already prepares it. If needed, rerun `cargo run --bin agent-audit -- prepare-tooling --run-id <run_id>` or `cargo run --bin agent-audit -- prepare-slither --run-id <run_id>`.
